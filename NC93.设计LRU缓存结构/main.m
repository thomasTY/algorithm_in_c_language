...
typedef struct lru { // 整个缓存的结构体
    int size; // 缓存大小k
    int count; // 缓存中有效数据计数
    Node *head; // 缓存的头结点，需要被分配空间，但不保存key-value
    Node *tail; // 缓存的尾结点，需要被分配空间，但不保存key-value
} LRU;
...
void init(int k) {
    ...
    db.head = (Node *)malloc(sizeof(Node)); // 创建缓存数据头结点
    db.tail = (Node *)malloc(sizeof(Node)); // 创建缓存数据尾结点
    // 缓存数据头尾结点连接起来，便于后续操作
...
void deleteTail(void) {
    Node *last_node = db.tail->prev; // 尾结点的前一个就是最后一个有效数据，这就是设置尾结点的方便之处
...
int getValue(int key) {
    Node *ptr = getNode(key); // 具体查找工作由上面辅助函数getNode()来做
...
void setValue(int key, int value) {
    Node *ptr = getNode(key); // 先查找缓存中是否已经存在要插入的key
    ...
    if (ptr == NULL) { // 若不存在，则创建一个新的缓存节点并***缓存
        ...
        ptr->value = value; // 更新缓存中的数据
    }
}
 
// operators整型二维数组，可包含多个操作，如：[[1,1,2],[2,key,0]] = [(赋值,key是1,值是2), 取值key的值]
// opsRowLen包含的行数；opsColLen包含的列数；
// k设置缓存容量；returnSize返回值的元素个数；返回值为一维数组
int* Lru(int** operators, int opsRowLen, int* opsColLen, int k, int* returnSize ) {
    int i, *op, *res = NULL;
 
    init(k);
    *returnSize = 0;
 
    for (i = 0; i < opsRowLen; i++) { // 遍历所有操作
